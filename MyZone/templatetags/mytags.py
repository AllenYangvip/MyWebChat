...
def bulid_comment_tree(itemid):
    ...
    for k, v in comment_dic.items():
        html += "<div coment-id ='%s'> " %k.id+"<span><span class='userName'>"+k.user.name+"</span></span> :<a>"+k.comment + "</a></div> "
...
def generate_comment_html(sub_comment_dic,margin_left_val):
    ...
    for k, v_dic in sub_comment_dic.items():  # 循环穿过来的字典
        htm += "<div style='margin-left:%spx'  class='comment-node'coment-id ='%s'>" % (margin_left_val,k.id) +"<span>|--<span class='userName'>"+k.user.name+"</span> : "+"</span><a>"+ k.comment + "</a></div>"
